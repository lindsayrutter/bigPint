...
Below, we read in the soybean iron metabolism dataset and perform a logarithm on it [@soybeanIR] dataset. We determine that this dataset contains 5,604 genes. We also determine that this dataset has 113 genes that have FDR < 1e-7 (n = 113). We will consider these our 113 significant genes of interest.
...
Running the command above will save the following files into the `outDir` location (current working directory by default): 
...
```{r, eval=FALSE, include=TRUE, message=FALSE, warning=FALSE}
S1S3Cluster1 <- readRDS("S1_S3_4_1.rds")
```
...
```{r, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE}
ret <- plotSM(data = soybean_cn_sub_st, geneList = S1S3Cluster1, pointColor = "#00A600FF", saveFile = FALSE)
