...
```{r, eval=TRUE, include=FALSE, message=FALSE, warning=FALSE, fig.width = 7.2916667, fig.asp= 1/1.618}
ret <- plotClusters(data_st, dataMetrics, threshVal = 0.1, nC = 2,
...
Below are the corresponding code blocks from above that now use the `SummarizedExperiment` object instead of the `data` and `dataMetrics` objects.

### Step 1: Side-by-side boxplot

Note that since we are not superimposing any metrics, we do not call the `rowData()` command.
