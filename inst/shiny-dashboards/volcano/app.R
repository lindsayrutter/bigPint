...
hoverinfo: 'text',
  ...
  observe({
    ...
    session$sendCustomMessage(type = "points", message=list(geneX=geneX, geneY=geneY, geneID=geneID, pointSize = pointSize))
  })
...
hoverinfo: 'text',
  ...
  observe({
    ...
    geneID <- curPairSel()[["ID"]]
    session$sendCustomMessage(type = "lines", message=list(pcpDat=degData, nVar = nVar, colNms = colNms, geneID=geneID))
  })
