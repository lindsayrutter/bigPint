helperTestData <- function(data){
  ...
  if (!class(data) == "data.frame"){ 
    ...
    stop("First column of data object must be called 'ID'.")
  }
  else if (!class(data[,1]) == "character"){
    stop("First column of data object must be of class 'character'.")    
  }
  else if (anyDuplicated(data[,1])>0){
    stop("First column of data object must contain unique IDs.") 
  }
  else if (ncol(data) < 5){
    stop("There must be at least five columns in the data object.
    ...
    stop("All columns but the first must be of class 'integer' or
         'numeric' in the data object.")
  }
  else if (all(logicPerl != c(1:length(colNames)))){
    stop("In the data object, the names of all columns but the first
         must match the Perl expression '^[a-zA-Z0-9]+\\.[0-9]+'")
  }
  else if (length(uGroups) < 2){
    stop("There must be at least two treatment groups in the
    data object.")
  }
  else if (!all(logicReps == TRUE)){
    stop("Each treatment group must have at least two replicates
    in the data object.")
  }
}
